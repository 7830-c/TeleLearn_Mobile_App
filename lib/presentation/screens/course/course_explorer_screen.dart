...
class _CourseExplorerScreenState extends State<CourseExplorerScreen> {
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (newTitle.isNotEmpty) {
                ...
                await context
                    .read<CourseProvider>()
                    .renameModule(widget.courseId, module.id, newTitle);
                if (mounted) {
                  ToastUtils.showSnackBar(context, 'Module renamed',
                      isSuccess: true);
                }
              }
            },
  ...
  Future<void> _handleSync() async {
    ...
    ToastUtils.showSnackBar(
      context,
      moduleId == null
          ? 'Syncing all lectures and notes from Telegram...'
          : 'Refreshing this module from Telegram...',
      isSuccess: true,
    );
    try {
      ...
      if (moduleId == null) {
        ...
        await courseProvider.refreshModule(widget.courseId, moduleId,
            phone: phone);
      }
      if (mounted) {
        ToastUtils.showSnackBar(
          context,
          moduleId == null
              ? 'Course fully synchronized with Telegram!'
              : 'Module refreshed successfully!',
          isSuccess: true,
        );
      }
    } catch (e) {
                ...
                builder: (context) {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, idx) {
                      ...
                      final completedCount = mod.lessons
                          .where((l) => progressProvider.isLessonCompleted(
                              course.id, l.id))
                          .length;
                    ...
                    return ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isCompleted = progressProvider.isLessonCompleted(
                            course.id, lesson.id);
                        final savedSec = progressProvider
                            .getLessonProgressSeconds(course.id, lesson.id);
                        final lastWatchedId =
                            progressProvider.getLastWatchedLessonId(course.id);
                        final isLastWatched = (lesson.id == lastWatchedId);
                        final progressPct =
                            progressProvider.getLessonProgressPercent(course.id,
                                lesson.id, lesson.duration?.toInt() ?? 0);
                        final progressFrac =
                            progressProvider.getLessonProgressFraction(
                                course.id,
                                lesson.id,
                                lesson.duration?.toInt() ?? 0);
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          Text(
                                            DurationFormatter.formatFileSize(
                                                note.size ?? 3145728),
                                            style: GoogleFonts.inter(
                                              fontSize: 11,
                                              color: isDark
